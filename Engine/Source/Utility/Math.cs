...
public static class Mathf
{
	...
	*/
	...
	{
		//TODO 不知道这里是否是正确的
		var radians = Calc.DegToRad * angleInDegrees;// MathHelper.ToRadians(angleInDegrees);
		return new Vector2
		{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static uint FloatToSortable(float v)
    {
        uint x = BitConverter.SingleToUInt32Bits(v);
        return (x & 0x8000_0000u) != 0 ? ~x : (x ^ 0x8000_0000u);
    }
	...
	public static bool IsInsideCircle(Vector2 source,Vector2 target,float radius,out float sqDistance)
	{
		sqDistance = (source - target).LengthSquared();
